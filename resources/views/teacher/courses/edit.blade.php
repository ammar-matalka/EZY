...
<div class="max-w-4xl">
    ...
    <div class="bg-white rounded-lg shadow-md p-8">
        <h2 class="text-2xl font-bold text-primary mb-6">Edit Course: {{ $course->title }}</h2>
        ...
        <form action="{{ route('teacher.courses.update', $course) }}" method="POST" enctype="multipart/form-data">
            ...
            @if($course->image)
                <div class="mb-6">
                    ...
                    <img src="{{ Storage::url($course->image) }}" alt="{{ $course->title }}" class="w-40 h-40 rounded-lg object-cover border border-gray-300">
                </div>
            @endif
            ...
            <div class="mb-6">
                <label class="block text-sm font-medium text-gray-700 mb-2">
                    {{ $course->image ? 'Change' : 'Upload' }} Course Image
                </label>
                <input
                    type="file"
                    name="image"
            ...
            @if($course->enrollments->count() > 0)
                <div class="bg-yellow-50 border border-yellow-300 rounded-lg p-4 mb-6">
                    <div class="flex items-start">
                        ...
                        <div>
                            <h4 class="text-sm font-semibold text-yellow-800 mb-1">Course has active students</h4>
                            <p class="text-sm text-yellow-700">
                                This course has <strong>{{ $course->enrollments->count() }} enrolled students</strong>.
                                Major changes may affect their learning experience.
                            </p>
                        </div>
                    </div>
                </div>
            ...
            <div class="flex items-center justify-end space-x-4 pt-6 border-t border-gray-200">
                <a href="{{ route('teacher.courses.index') }}" class="px-6 py-2 bg-gray-200 hover:bg-gray-300 text-gray-700 rounded-lg font-medium transition">
                    Cancel
                </a>
